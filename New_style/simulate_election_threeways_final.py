...
def mle(data):
    ...
    #optimize the log likelihood using Minka's fixed point algorithm
    result = minka_mle_polya(data)
    #improve the optimization a little bit by using BFGS
    mle[nonzero_columns] = result
...
for column_to_group in data.columns[cluster_group]:  
    ...
    sims.to_pickle(fout + "_" + "nsims"+str(nsims) + "mlepriorclusteredby_" +column_to_group + ".pkl")
else:
    ...
    sims = pd.Panel(sims)
    sims.to_pickle(fout + "_" + "nsims"+str(nsims) + "jeffrysprior" +column_to_group + ".pkl")
