...
for pvalue in pvalues:
    ...
    for el in election_types:
        ...
        for year in election_years:
            ...
            keys = [key for key in all.keys() if year in key]
            for key in keys:    #e.g. key = all.keys()[0]
                ...
                for party in parties:
                    ...
                    accept_election.append(pObs < pvalue)
                    points.append((i,pObs,party))
                    if pObs >= pvalue:
                        text.append((i,1.05,key[0:2]))
                    i = i+1
            np.mean(accept_election)
            ax.plot([x[0] for x in points if "d" in x[2]],[y[1] for y in points if "d" in y[2]],"r^",color="blue") #plot dems
            ax.plot([x[0] for x in points if "r" in x[2]],[y[1] for y in points if "r" in y[2]],"r^",color="blue") #plot dems
        ...
        for year in election_years:
           ...
           #set x-axis to start at wherever we are on the x-axis to begin with
